"""
Qwen3-1.7B 원본 모델 블라인드 테스트 스크립트 (blind_test_base_1.7b.py)

파인튜닝하지 않은 원본 Qwen3-1.7B 모델의 성능을 테스트합니다.
...
MODEL_NAME = "unsloth/Qwen3-1.7B-unsloth-bnb-4bit"  # 원본 모델
TEST_DATA_PATH = PROJECT_ROOT / "data/raw/train_final1.jsonl"
OUTPUT_PATH = PROJECT_ROOT / "data/blind_test_result_base.md"
MAX_SEQ_LENGTH = 2048
...
# Instruction (블라인드 버전 - 원본 모델용 상세 지시)
...
def main():
    print("=" * 60)
    print("원본 Qwen3-1.7B 블라인드 테스트 (파인튜닝 전)")
    print("=" * 60)
    ...
    md_output = []
    md_output.append(f"# 블라인드 테스트 결과 (원본 모델)\n")
    md_output.append(f"> 테스트 일시: {now}\n")
    md_output.append(f"> 모델: `{MODEL_NAME}` (파인튜닝 전 원본)\n")
    ...
    for r in results:
        ...
        result_emoji = "O" if r['is_correct'] else "X"
        md_output.append(
    ...
    print("=" * 60)
    print(f"\n결과 저장됨: {OUTPUT_PATH}")
    print("파인튜닝 모델 결과(73.9%)와 비교해보세요!")
